    ...
    if (isinstance(map,tuple)):
        if (isinstance(map[0],tuple)):
            for iReplace in range(0,len(replaced)):
                map[0][0].genes[replacedI[iReplace], replacedJ[iReplace]][:] = genes[replacement[iReplace]]
            # print("map[0][0].genes")
            # print(map[0][0].genes)
            # print(map[0][0].genes.shape)
        else:
            for iReplace in range(0,len(replaced)):
                map[0].genes[replacedI[iReplace], replacedJ[iReplace]][:] = genes[replacement[iReplace]]
            # print("map[0].genes")
            # print(map[0].genes)
            # print(map[0].genes.shape)
    else:
